<template>
    <div class="dropdown">
        <div 
        @click="changeState" 
        class="dropdown-btn sidebar-dropdown-btn" 
        :class="{
            'active': isActive
        }">
            <FontAwesomeIcon :icon="faCaretRight" /> {{ title }}
        </div>
        <div class="dropdown-content sidebar-content scrollbar" v-show="isActive">
            <slot></slot>
        </div>
    </div>
</template>
